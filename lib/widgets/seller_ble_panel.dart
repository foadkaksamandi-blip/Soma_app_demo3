  ...
  Future<void> _toggle() async {
    try {
      if (!_advertising) {
        ...
        if (!ok) {
          setState(() => _status = 'اجازه‌های BLE صادر نشد');
          return;
        }
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          const Text('بلوتوث فروشنده', style: TextStyle(fontWeight: FontWeight.w600)),
          const SizedBox(height: 8),
          Text('وضعیت: $_status'),
          const SizedBox(height: 12),
          ElevatedButton(
            onPressed: _toggle,
            child: Text(_advertising ? 'توقف پخش BLE' : 'شروع پخش BLE'),
          ),
        ]),
      ),
    );
